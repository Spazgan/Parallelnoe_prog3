...
//    return 0;
//}
//Пример 12 демонстрирует использование опции private.В данном примере
//переменная n объявлена как локальная переменная в параллельной области.
//Это значит, что каждая нить будет работать со своей копией переменной n,
//при этом в начале параллельной области на каждой нити переменная n не будет инициализирована.В ходе выполнения программы значение переменной
//n будет выведено в четырёх разных местах.Первый раз значение n будет выведено в последовательной области, сразу после присваивания переменной n
//30 значения 1. Второй раз все нити выведут значение своей копии переменной n в начале параллельной области, неинициализированное значение может зависеть от реализации.Далее все нити выведут свой порядковый номер,
//полученный с помощью функции omp_get_thread_num() и присвоенный
//переменной n.После завершения параллельной области будет ещё раз выведено значение переменной n, которое окажется равным 1 (не изменилось во
//	время выполнения параллельной области).
...
//Пример 13 демонстрирует использование опции shared
// .Массив m объявлен общим для всех нитей.
// В начале последовательной области массив m заполняется нулями и выводится на печать.В параллельной области каждая
...
//    return 0;
//}
//Пример 14 демонстрирует использование опции firstprivate.Переменная n объявлена как firstprivate в параллельной области.
// Значение n будет выведено в четырёх разных местах.Первый раз значение n будет выведено в последовательной области сразу после инициализации.Второй раз все
...
//переменной n выводится в четырёх разных местах.
// Первый раз все нити выведут значение своей копии переменной n в начале параллельной области, и
//это значение будет равно 1 на нити - мастере и 0 на остальных нитях.Далее с
//помощью функции omp_get_thread_num() все нити присвоят переменной n свой порядковый номер и выведут это значение.
//Затем в последовательной области будет ещё раз выведено значение переменной n, которое
